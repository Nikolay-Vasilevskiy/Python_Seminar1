...
# Дополнительное условие - выбирать 0 запрещено, иначе игра уйдет с бесконечность
...
# Жеребьевка
def lotteri ():
    name11 = (input('Ведите имя игрока: '))
    name1 = name11.upper()
...
# Делаем ход в игре бота
def make_move_bot (count, max):
    print('\nХод БОТА.')
    if count > max:
        player = logika(count, max)
        if player == 0:
            player = randint(1,28) # Замена 0 для БОТА на случайное число
        count = count - player   # Подсчет остатка конфет 
        print('БОТ выбрал ', player, ' конфет.')
        print('Осталось ',count,' конфет')
    else: 
        player = count
        count = count - player   # Подсчет остатка конфет 
